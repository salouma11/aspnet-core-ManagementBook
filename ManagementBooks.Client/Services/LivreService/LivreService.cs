...
namespace ManagementBooks.Client.Services.LivreService
{
    public class LivreService : ILivreService
    {
        ...
        public async Task AddLivreWithAll(Livre liv)
        {
            var result = await _httpClient.PostAsJsonAsync("api/Livre", liv);
        ...
        public async Task UpdateLivreWithCategoriesAsync(Livre livre)
        {
            var result = await _httpClient.PutAsJsonAsync($"api/Livre/{livre.Id}", livre);
            var response = await result.Content.ReadFromJsonAsync<List<Livre>>();
